 ...
 *
 *
 * authc 过滤器：要求用户必须进行登录认证，否则会被重定向到登录页面。适用于用户必须登录后才能访问的资源。
 *
 * user 过滤器：允许用户访问，用户可以是通过记住我功能登录的，但不一定经过完整认证。
 *
 * 常用 Shiro 认证过滤器介绍
 * anon：匿名过滤器，表示资源可以不经过认证即可访问。
 * authc：表示用户必须登录才能访问，如果未登录会重定向到登录页面。
 * authcBasic：使用 HTTP Basic 认证。
 * user：表示用户不一定要登录，但如果通过“记住我”功能登录，可以访问资源。
 * roles：基于角色的访问控制过滤器，只有具备特定角色的用户才能访问。
 * perms：基于权限的访问控制过滤器，只有具备特定权限的用户才能访问。
 * </p>
 *
 * <p>
 * authen: hkk
...
public class Shiro2Config {
    ...
    public ShiroFilterFactoryBean getShiroFilterFactoryBean(DefaultWebSecurityManager defaultWebSecurityManager, ILicenseService licenseService) {
         ...
         */
        HashMap<String, Filter> filters = new HashMap<>(16);
        filters.put("licenseFilter", createLicenseFilter(licenseService));
...
//        filterChainDefinitionMap.put("/admin/**", "anon");
        ...
        // 将过滤器链定义设置到 ShiroFilterFactoryBean
